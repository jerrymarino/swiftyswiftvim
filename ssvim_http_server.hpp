...
public:
  ssvi_http_server(endpoint_type const &ep, std::size_t threads,
                   std::string const &root)
      : _acceptor(_io_service), _socket(_io_service), _root_path(root) {
    ...
    for (std::size_t i = 0; i < threads; ++i)
      _thread.emplace_back([&] { _io_service.run(); });
  }

  ~ssvi_http_server() {
    error_code ec;
    _io_service.dispatch([&] { _acceptor.close(ec); });
    for (auto &t : _thread)
      t.join();
  }
...
private:
  ...
  static void async_write(Stream &stream,
                          message<isRequest, Body, Fields> &&msg,
                          DeducedHandler &&handler) {
    write_op<Stream, typename std::decay<DeducedHandler>::type, isRequest, Body,
